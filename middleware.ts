...
export const config = {
  matcher: '/passport/public/:username*',
};
...
export default async function middleware(req: Request) {
  ...
  if (!profile) return; // unknown user, fall back to default SPA index.html
  ...
  const title = `${profile.name} — ${profile.headline || profile.title || 'Professional Career Passport'} | TalentXcel`;
  const description = profile.summary?.slice(0, 200) || `${profile.name}'s verified professional passport on TalentXcel.`;
