...
export class EvidenceStore {
  ...
  public static getEvidenceCoverage(
    totalQueries: number = 0,
    totalIntents: number = 0,
    verifiedJobsCount: number = 16,
    liveJobsCount: number = 456
  ): WorldEvidenceCoverage {
    const serpObs = totalQueries; // Dynamic GSC rank observations
    ...
    const supplyObs = verifiedJobsCount; // First-party partner verified jobs (16)
    const liveObs = liveJobsCount; // Total active live vacancies (456)
    ...
    return {
      ...
      liveSupplyObservations: liveObs,
