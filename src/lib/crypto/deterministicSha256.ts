// src/lib/crypto/deterministicSha256.ts
// Pure TypeScript FIPS 180-4 SHA-256 implementation
// 100% Browser & Node.js compatible with zero external dependencies.
// Eliminates Rollup/Vite "crypto externalized for browser compatibility" bundling errors.
...
export function sha256(ascii: string): string {
  ...
  for (let b = 0; b < utf8Bytes.length; b += 4) {
    words.push((utf8Bytes[b] << 24) | (utf8Bytes[b + 1] << 16) | (utf8Bytes[b + 2] << 8) | utf8Bytes[b + 3]);
  }
  ...
  for (let chunk = 0; chunk < words.length; chunk += 16) {
    ...
    for (i = 0; i < 64; i++) {
      if (i < 16) {
        ...
        const gamma0 = ((w[i - 15] >>> 7) | (w[i - 15] << 25)) ^ ((w[i - 15] >>> 18) | (w[i - 15] << 14)) ^ (w[i - 15] >>> 3);
        const gamma1 = ((w[i - 2] >>> 17) | (w[i - 2] << 15)) ^ ((w[i - 2] >>> 19) | (w[i - 2] << 13)) ^ (w[i - 2] >>> 10);
...
export function sha256Truncated(input: string, length: number = 10): string {
  return sha256(input).slice(0, length);
}

export function deterministicId(prefix: string, ...parts: string[]): string {
  const hash = sha256Truncated(parts.join('|'), 10);
