...
export function generateClusterId(normalizedQuery: string, surface: string): string {
  const hash = createHash('sha256').update(`${normalizedQuery}|${surface}`).digest('hex').slice(0, 8);
  return `cls_${hash}`;
}
...
export const SAMPLE_INTENT_CLUSTERS: IntentCluster[] = [
  // JOBS surface - 4 clusters
  {
    ...
    dedup_hash: createHash('md5').update('software engineer bangalore JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('data analyst delhi JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('content writer noida JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('marketing executive noida JOBS').digest('hex'),
  ...
  // RESUME_ATS surface - 2 clusters
  {
    ...
    dedup_hash: createHash('md5').update('ats resume scanner RESUME_ATS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_TOOL',
    journey_stage: 'DECISION',
    dedup_hash: createHash('md5').update('resume builder software engineer RESUME_ATS').digest('hex'),
  ...
  // COLLEGES surface - 2 clusters
  {
    ...
    dedup_hash: createHash('md5').update('engineering colleges india placement COLLEGES').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'INFORMATIONAL_EDUCATION',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('mba colleges bangalore COLLEGES').digest('hex'),
  ...
  // PROFESSIONAL_NETWORK surface
  {
    ...
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('professional networking india PROFESSIONAL_NETWORK').digest('hex'),
  ...
  // ROLE_GUIDES surface - 2 clusters
  {
    ...
    dedup_hash: createHash('md5').update('data scientist career india ROLE_GUIDES').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'INFORMATIONAL_EDUCATION',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('product manager salary india ROLE_GUIDES').digest('hex'),
  ...
  // SKILLS surface
  {
    ...
    intent_category: 'INFORMATIONAL_EDUCATION',
    journey_stage: 'DISCOVERY',
    dedup_hash: createHash('md5').update('python course india SKILLS').digest('hex'),
  ...
  // LOCATIONS surface
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('it jobs hyderabad LOCATIONS').digest('hex'),
  ...
  // COMPANIES surface
  {
    ...
    intent_category: 'CAREER_GUIDANCE',
    journey_stage: 'DECISION',
    dedup_hash: createHash('md5').update('infosys interview COMPANIES').digest('hex'),
  ...
  // LEARNING_COURSES surface
  {
    ...
    intent_category: 'TRANSACTIONAL_TOOL',
    journey_stage: 'DECISION',
    dedup_hash: createHash('md5').update('digital marketing course LEARNING_COURSES').digest('hex'),
  ...
  // CAREER_MAP surface
  {
    ...
    intent_category: 'CAREER_GUIDANCE',
    journey_stage: 'DISCOVERY',
    dedup_hash: createHash('md5').update('dev to pm career switch CAREER_MAP').digest('hex'),
    estimated_cluster_volume: 12000,
  ...
  // CAREER_PASSPORT surface
  {
    ...
    intent_category: 'TRANSACTIONAL_TOOL',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('verified digital portfolio CAREER_PASSPORT').digest('hex'),
  ...
  // MO1_BUSINESS_OS surface
  {
    ...
    intent_category: 'COMMERCIAL_B2B',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('ai recruitment enterprise COMMERCIAL_B2B').digest('hex'),
  ...
  // BIDDER_RANKINGS surface
  {
    ...
    intent_category: 'INFORMATIONAL_EDUCATION',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('ai products leaderboard BIDDER_RANKINGS').digest('hex'),
  ...
  // CAREER_TOOLS surface
  {
    ...
    intent_category: 'TRANSACTIONAL_TOOL',
    journey_stage: 'DECISION',
    dedup_hash: createHash('md5').update('salary calculator india CAREER_TOOLS').digest('hex'),
  ...
  // Additional JOBS clusters to round to 25
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('freshers jobs 2026 JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('remote work from home jobs JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'TRANSACTIONAL_JOB',
    journey_stage: 'APPLICATION',
    dedup_hash: createHash('md5').update('engineering internship india JOBS').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'CAREER_GUIDANCE',
    journey_stage: 'DECISION',
    dedup_hash: createHash('md5').update('interview questions india ROLE_GUIDES').digest('hex'),
  ...
  },
  {
    ...
    intent_category: 'CAREER_GUIDANCE',
    journey_stage: 'EVALUATION',
    dedup_hash: createHash('md5').update('linkedin profile optimize PROFESSIONAL_NETWORK').digest('hex'),
