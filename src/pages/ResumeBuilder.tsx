...
const ResumeBuilder = () => {
  ...
  const generateResumeWithAI = async () => {
    ...
    // In a real implementation, this would call a Transformers.js model
    // For now, we'll generate a mock professional summary
    const mockGeneratedSummary = `Results-driven ${resumeData.personalInfo.summary || 'professional'} with proven experience in ${resumeData.skills.slice(0, 3).join(', ')}. Demonstrated ability to deliver high-quality results in fast-paced environments. Strong problem-solving skills and collaborative approach to achieving organizational goals.`;
    
    const mockResumeHTML = `
      <div class="resume-container" style="max-width: 800px; margin: 0 auto; padding: 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333;">
        ...
        <section style="margin-bottom: 30px;">
          ...
          <p style="font-size: 1.1em; line-height: 1.7;">${mockGeneratedSummary}</p>
        </section>
        
        ${resumeData.skills.length > 0 ? `
        <section style="margin-bottom: 30px;">
          <h2 style="font-size: 1.5em; color: #3B82F6; border-bottom: 2px solid #E5E7EB; padding-bottom: 5px; margin-bottom: 15px;">Core Skills</h2>
          <div style="display: flex; flex-wrap: wrap; gap: 8px;">
            ${resumeData.skills.map(skill => `<span style="background: #F3F4F6; color: #374151; padding: 6px 12px; border-radius: 20px; font-size: 0.9em;">${skill}</span>`).join('')}
          </div>
        </section>
        ` : ''}
          ...
          ${resumeData.experience.filter(exp => exp.title || exp.company).map(exp => `
            <div style="margin-bottom: 25px;">
              <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 8px;">
                <h3 style="font-size: 1.2em; margin: 0; color: #1F2937;">${exp.title}</h3>
                <span style="font-size: 0.9em; color: #6B7280;">${exp.startDate} - ${exp.current ? 'Present' : exp.endDate}</span>
              </div>
              <div style="font-weight: 600; color: #3B82F6; margin-bottom: 8px;">${exp.company}${exp.location ? ` • ${exp.location}` : ''}</div>
          ...
          ${resumeData.education.filter(edu => edu.degree || edu.school).map(edu => `
            <div style="margin-bottom: 20px;">
              <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 5px;">
                <h3 style="font-size: 1.1em; margin: 0; color: #1F2937;">${edu.degree}</h3>
                <span style="font-size: 0.9em; color: #6B7280;">${edu.startDate} - ${edu.endDate}</span>
              </div>
              <div style="color: #3B82F6; font-weight: 600;">${edu.school}${edu.location ? ` • ${edu.location}` : ''}</div>
    ...
    setGeneratedResume(mockResumeHTML);
              ...
              <CardContent>
                {generatedResume ? (
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Fill in your information and click "Generate with AI"</p>
                  </div>
                )}
              </CardContent>
            </Card>
