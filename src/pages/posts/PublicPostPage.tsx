...
export default function PublicPostPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50/60 dark:bg-slate-950 py-12 px-4">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-2xl p-6 space-y-4 shadow-sm">
  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-slate-50/60 dark:bg-slate-950 py-20 px-4 text-center">
        <div className="max-w-md mx-auto space-y-4 bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 shadow-sm">
      ...
      <div className="min-h-screen bg-slate-50/60 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-8 px-4 pb-20">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <nav aria-label="Breadcrumb" className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium">
            ...
            <Link to="/network" className="hover:text-blue-600 transition-colors">Network</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-blue-600 font-semibold truncate max-w-[200px]">{headline}</span>
          </nav>

          {/* Main Post Article Card - High Contrast White Card */}
          <article className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
            {/* Header: Author & Date */}
            <div className="flex items-start justify-between gap-4">
              <Link to={`/@${authorUsername}`} className="flex items-center gap-3.5 group">
                ...
                <div>
                  <div className="font-bold text-slate-900 dark:text-white text-sm sm:text-base group-hover:text-blue-600 transition-colors flex items-center gap-1.5">
                    {authorName}
                    <ShieldCheck className="w-4 h-4 text-blue-600" />
                  </div>
                  <div className="text-xs text-slate-500 font-medium">{authorTitle}</div>
              ...
              <Button
                ...
                className="border-slate-200 dark:border-slate-700 hover:bg-slate-100 text-xs text-slate-700 dark:text-slate-200 font-semibold gap-1.5 h-8"
              >
            ...
            {/* Post Content Body - Pure High Contrast Readability */}
            <div className="text-slate-700 dark:text-slate-200 text-sm sm:text-base leading-relaxed whitespace-pre-line break-words font-normal">
              {cleanContent}
            </div>
                ...
                {post.media_urls.map((url: string, idx: number) => {
                  ...
                  return isVideo ? (
                    <video
                      ...
                      src={url}
                      controls
                      className="rounded-xl border border-slate-200 max-h-[500px] w-full bg-black"
                    />
                  ) : (
                    <img
                      key={idx}
                      src={url}
                      alt="Post attachment"
                      className="rounded-xl border border-slate-200 max-h-[500px] object-cover w-full shadow-xs"
                    />
                  );
                })}
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 flex-wrap">
              ...
              {topics.map((t) => (
                <Link key={t.slug} to={`/topics/${t.slug}`}>
                  <Badge variant="secondary" className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-blue-600 text-xs font-medium border border-slate-200 dark:border-slate-700">
                    {t.label}
                  </Badge>
                </Link>
              ))}
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500">
              <div className="flex items-center gap-4 font-semibold">
                <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-400">
                  <ThumbsUp className="w-3.5 h-3.5 text-blue-600" /> {post.likes_count || 0} Likes
                </span>
                <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-400">
                  <MessageCircle className="w-3.5 h-3.5 text-purple-600" /> {post.comments_count || 0} Comments
                </span>
              </div>

              <Link to="/network">
                <Button size="sm" variant="ghost" className="text-blue-600 hover:text-blue-700 text-xs font-semibold gap-1 h-7">
