// src/agents/core/TaskScheduler.ts
// Autonomous Operating Heartbeat & Scheduled Pulse Engine

import { coreAgentOrchestrator } from './AgentOrchestrator';
import { kernelTaskQueue } from '../kernel/TaskQueue';
import { kernelToolRegistry } from '../kernel/ToolRegistry';
...
export class TaskScheduler {
  ...
  private readonly TICK_INTERVAL_MS = 60_000; // 60s standard pulse

  start() {
    if (this.isRunning) return;
    this.isRunning = true;
    console.log('⚡ [TaskScheduler] Autonomous 60-Second Operating Loop Activated.');
    this.tick();
  ...
  stop() {
    ...
    console.log('🛑 [TaskScheduler] Autonomous Loop Paused.');
  }
  ...
  async tick() {
    ...
    try {
      // 1. Process asynchronous tasks in queue
      ...
      console.error('[TaskScheduler] Tick error:', err);
    }
  }
...
export const coreTaskScheduler = new TaskScheduler();
