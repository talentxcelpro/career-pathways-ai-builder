...
export const AICareerNavigator: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      type: 'Navigator',
      content: "Hi! I'm your AI Career Navigator. I can help you with job search strategies, resume optimization, interview preparation, and career planning. What would you like to work on today?",
      timestamp: new Date(),
      suggestions: [
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'insights' | 'actions' | 'CareerAnalytics'>('chat');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await chatWithDeepSeek(
        inputMessage,
        "You are an AI Career Navigator specialized in helping job seekers with resume optimization, interview preparation, career planning, and job search strategies. Provide practical, actionable advice."
      );
    ...
    <Card className="h-[600px] flex flex-col">
      <CardHeader>
        ...
        <div className="flex gap-2">
          {['chat', 'insights', 'actions', 'CareerAnalytics'].map((tab) => (
            <Button
              key={tab}
      ...
      <CardContent className="flex-1 flex flex-col">
        ...
        {activeTab === 'CareerAnalytics' && (
          <div className="space-y-6">
            <div className="flex items-center gap-2 mb-4">
              <BarChart className="h-5 w-5 text-purple-600" />
              <span className="font-semibold">Career CareerAnalytics</span>
            </div>
