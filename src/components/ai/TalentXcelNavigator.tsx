...
const TalentXcelNavigator: React.FC = () => {
  const { user } = useAuth();
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      content: "👋 Hi! I'm your TalentXcel Navigator. I'm here to help you navigate your career journey with personalized signals and actionable advice. What career challenge can I help you with today?",
      timestamp: new Date(),
      suggestions: [
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (data.insights) {
        toast.success('New career signals generated!');
      }

    } catch (error) {
      console.error('Error sending message to TalentXcel Navigator:', error);
      toast.error('Failed to get Navigator response');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold">TalentXcel Navigator</h2>
          <p className="text-muted-foreground">Get personalized career guidance powered by Talent Engine</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-primary/20">
          <CardContent className="p-4 text-center">
            ...
            <h3 className="font-semibold mb-1">Personalized Signals</h3>
            <p className="text-sm text-muted-foreground">Talent Engine-driven career moves</p>
          </CardContent>
        </Card>
...
export default TalentXcelNavigator;
