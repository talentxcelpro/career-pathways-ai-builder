...
interface OptimizedSocialPulseProps {
  PulseType?: 'explore' | 'following' | 'trending';
  userId?: string;
}
...
export const OptimizedSocialPulse = memo<OptimizedSocialPulseProps>(({ 
  PulseType = 'explore',
  userId 
}) => {
  ...
  }, [PulseType, userId, page]);

  const { data: posts = [], isLoading, error } = useOptimizedQuery({
    queryKey: ['social-Pulse', PulseType, userId, page],
    queryFn: fetchPosts,
...
OptimizedSocialPulse.displayName = 'OptimizedSocialPulse';
