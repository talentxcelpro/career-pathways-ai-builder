...
interface NetworkPostsPulseProps {
  PulseType?: 'all' | 'smart';
}

export const NetworkPostsPulse: React.FC<NetworkPostsPulseProps> = ({ 
  PulseType = 'all' 
}) => {
  const { user } = useAuth();
  ...
  const { data: posts = [], isLoading, error, refetch } = useQuery({
    queryKey: ['network-posts', PulseType],
    queryFn: async () => {
      if (!user) return [];
      ...
      // Apply smart Pulse filtering if needed
      if (PulseType === 'smart') {
        // Get user's TalentNetwork for personalization
        ...
        // Prioritize posts from TalentNetwork and recent posts
      ...
      <Card className="bg-card/95 backdrop-blur-sm border-destructive/20">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-muted-foreground mb-4">
            We couldn't load the network Pulse. Please try again.
          </p>
          <button
            onClick={() => refetch()}
        ...
        <CardContent className="p-6 text-center">
          <div className="py-8">
            ...
            <p className="text-muted-foreground">
              {PulseType === 'smart' 
                ? "Your smart Pulse will show personalized content once there are more posts from your network."
