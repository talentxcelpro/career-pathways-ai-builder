...
export const useAdvancedPulseAlgorithm = (module: 'reels' | 'network' | 'jobs') => {
  const { user } = useAuth();
  const [userBehavior, setUserBehavior] = useState<UserBehaviorData | null>(null);
  const [personalizedPulse, setPersonalizedPulse] = useState<PulseItem[]>([]);
  ...
  const trackBehavior = useCallback(async (action: string, contentId: string, contentType: string, metadata?: any) => {
    ...
    try {
      // Store behavior data in a behavior_CareerAnalytics table
      await supabase.from('behavior_CareerAnalytics').insert({
        user_id: user.id,
        action,
  ...
  const calculateRelevanceScore = useCallback((item: PulseItem, behavior: UserBehaviorData): number => {
    let score = 0;
  ...
  // Generate personalized Pulse
  const generatePersonalizedPulse = useCallback(async () => {
    ...
    try {
            ...
            .map(post => {
              ...
              return {
                ...
                author: {
                  ...
                  isConnection: false, // Would check TalentNetwork table
      ...
      setPersonalizedPulse(scoredItems);
      console.log('🤖 Generated personalized Pulse:', scoredItems.length, 'items for', module);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadUserBehavior = async () => {
      try {
        // Aggregate user behavior from CareerAnalytics
        const { data: behaviors } = await supabase
          .from('behavior_CareerAnalytics')
          .select('*')
          .eq('user_id', user.id)
        ...
        if (behaviors && behaviors.length > 0) {
          ...
          setUserBehavior({
            ...
            modulePreferences,
            peakActivityHours,
            TalentNetworkimilarity: [], // Would load from TalentNetwork analysis
  ...
  // Generate Pulse when behavior data is loaded
  useEffect(() => {
    if (userBehavior) {
      generatePersonalizedPulse();
    }
  }, [userBehavior, generatePersonalizedPulse]);

  return {
    personalizedPulse,
    isLoading,
    trackBehavior,
    refreshPulse: generatePersonalizedPulse,
    userBehavior
  };
};
