...
import { useInfiniteNetworkPulse, NetworkPost } from '@/hooks/useInfiniteNetworkPulse';
import { VirtualizedNetworkPulse } from './VirtualizedNetworkPulse';
...
interface EnhancedMobilePulseProps {
  className?: string;
}

export const EnhancedMobilePulse: React.FC<EnhancedMobilePulseProps> = ({ className = '' }) => {
  ...
  const {
    ...
    isFetchingNextPage,
    refetch
  } = useInfiniteNetworkPulse({ type: filter });
  ...
  // Combine and shuffle content for engaging mixed Pulse
  const allContent = [
  ...
  const PostCard = React.memo(({ post }: { post: NetworkPost }) => {
    ...
    const { src, imgClasses } = useImageOptimizer(mediaUrl || '');
        ...
        {mediaUrl && (
          <div className="relative mb-3 overflow-hidden rounded-lg bg-muted">
            <img 
              src={src} 
              alt="Post content" 
              className={cn("w-full aspect-square object-cover object-center", imgClasses)}
              loading="lazy"
            />
            {post.post_type === 'video' && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                <div className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center shadow-elegant">
