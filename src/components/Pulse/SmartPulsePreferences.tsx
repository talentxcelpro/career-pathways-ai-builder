...
interface SmartPulsePreferencesProps {
  preferences: any;
...
export const SmartPulsePreferences: React.FC<SmartPulsePreferencesProps> = ({ 
  preferences, 
  onPreferencesChange 
}) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="border-0 shadow-card bg-card/50 backdrop-blur-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-heading-lg text-foreground font-display">Pulse Behavior</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 rounded-xl bg-background/40 hover:bg-background/60 transition-all duration-200">
              <div className="flex-1">
                <Label className="text-body font-display font-semibold text-foreground">Prioritize TalentNetwork</Label>
                <p className="text-caption text-muted-foreground mt-1 leading-relaxed">Show posts from your network first</p>
              </div>
              <Switch
                checked={preferences.prioritize_TalentNetwork !== false}
                onCheckedChange={(checked) => handleBehavioralSettingChange('prioritize_TalentNetwork', checked)}
                className="data-[state=checked]:bg-primary"
              />
            </div>
