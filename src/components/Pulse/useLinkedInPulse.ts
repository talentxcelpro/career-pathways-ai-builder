...
export const useLinkedInPulse = () => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      // Get current user's TalentNetwork
      ...
      const TalentNetwork = new Set();
      TalentNetworkData?.forEach(conn => {
        if (conn.requester_id === user.id) TalentNetwork.add(conn.recipient_id);
        if (conn.recipient_id === user.id) TalentNetwork.add(conn.requester_id);
      });
      ...
      const transformedPosts: LinkedInPost[] = postsData.map(post => {
        ...
        const isConnection = TalentNetwork.has(post.author_id);
