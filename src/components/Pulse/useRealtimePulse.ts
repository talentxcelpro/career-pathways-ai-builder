...
/**
 * Real-time Pulse Hook with Incremental Updates
 * Implements pub/sub pattern for live Pulse updates
 */
export const useRealtimePulse = () => {
  const { user } = useAuth();
  const [PulseItems, setPulseItems] = useState<PulseItem[]>([]);
  const [pendingUpdates, setPendingUpdates] = useState<PulseUpdate[]>([]);
  const [isConnected, setIsConnected] = useState(false);
  const [unreadCount, setUnreadCount] = useState(0);
  const PulseQueueRef = useRef<PulseUpdate[]>([]);
  const processingRef = useRef(false);

  // Pulse ranking algorithm
  const calculatePriority = useCallback((item: PulseItem): number => {
    const baseScore = {
      post: 10,
      like: 3,
      comment: 5,
  ...
  const processUpdateQueue = useCallback(async () => {
    if (processingRef.current || PulseQueueRef.current.length === 0) return;
    
    processingRef.current = true;
    const updates = [...PulseQueueRef.current];
    PulseQueueRef.current = [];

    try {
      // Batch process updates
      const itemsToAdd: PulseItem[] = [];
      const itemsToUpdate = new Map<string, PulseItem>();
      ...
      if (PulseQueueRef.current.length > 0) {
  ...
  const queueUpdate = useCallback((update: PulseUpdate) => {
    PulseQueueRef.current.push(update);
  ...
  const markAsSeen = useCallback((itemIds: string[]) => {
    ...
    setUnreadCount(prev => {
      const unseenItems = PulseItems.filter(item => 
  ...
  // Get Pulse items by type
  const getItemsByType = useCallback((type: PulseItem['type']) => {
    return PulseItems.filter(item => item.type === type);
  }, [PulseItems]);
