...
export const useReelsPulse = (PulseType: 'following' | 'explore' = 'explore') => {
  const { user } = useAuth();
  ...
  const mockPosts: ReelsPost[] = [
    {
      ...
      caption: 'Just shipped a major feature at work! 🚀 The feeling when your code works on the first try is unmatched. Here\'s a quick walkthrough of the new CommandCenter we built for our users. #TechLife #SoftwareEngineering #WebDevelopment',
      stats: {
        likes: 1547,
        comments: 89,
        shares: 23,
  ...
  useEffect(() => {
    const loadPulse = async () => {
      ...
      try {
        ...
        // Filter posts based on Pulse type
        let filteredPosts = mockPosts;
        if (PulseType === 'following') {
