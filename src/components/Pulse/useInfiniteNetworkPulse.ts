...
export function useInfiniteNetworkPulse(filters: PulseFilters = {}) {
  return useInfiniteQuery({
    queryKey: ['network-Pulse', filters],
    queryFn: async ({ pageParam = 0 }) => {
      ...
      if (filters.type === 'connections' && userId) {
        const [TalentNetworkRes, followingRes] = await Promise.all([
          supabase
            .from('connections')
        ...
        if (TalentNetworkRes.error) console.warn('Connection Pulse filter failed:', TalentNetworkRes.error);
        if (followingRes.error) console.warn('Following Pulse filter failed:', followingRes.error);

        const connectionIds = (TalentNetworkRes.data ?? [])
          .map((connection: any) => (connection.requester_id === userId ? connection.recipient_id : connection.requester_id))
          .filter(Boolean);
        const followingIds = (followingRes.data ?? []).map((follow: any) => follow.following_id).filter(Boolean);

        connectedUserIds = Array.from(new Set([...connectionIds, ...followingIds]));
      ...
      if (filters.type === 'trending') {
        query = query.order('likes_count', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false });
      } else {
        query = query.order('created_at', { ascending: false });
      }

      if (filters.type === 'connections' && connectedUserIds.length > 0) {
        query = query.in('author_id', connectedUserIds);
      }
      ...
      if (error) {
        console.error('useInfiniteNetworkPulse query error:', error);
        throw error;
      }
