...
interface RealtimePulseUpdatesProps {
  onRefreshPulse: () => void;
  className?: string;
}

export const RealtimePulseUpdates: React.FC<RealtimePulseUpdatesProps> = ({
  onRefreshPulse,
  className
}) => {
  const { user } = useAuth();
  const { PulseItems, unreadCount, isConnected, markAsSeen } = useRealtimePulse();
  const [showUpdates, setShowUpdates] = useState(false);
  const [autoRefresh, setAutoRefresh] = useState(true);

  // Auto-refresh Pulse when new items arrive (if enabled)
  useEffect(() => {
    if (autoRefresh && unreadCount > 0) {
      const timer = setTimeout(() => {
        onRefreshPulse();
        const visibleItemIds = PulseItems.slice(0, 5).map(item => item.id);
        markAsSeen(visibleItemIds);
      }, 2000); // 2 second delay for smooth UX

      return () => clearTimeout(timer);
    }
  }, [unreadCount, autoRefresh, onRefreshPulse, PulseItems, markAsSeen]);
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      {showUpdates && PulseItems.length > 0 && (
        <motion.div
