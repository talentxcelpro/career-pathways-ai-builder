...
interface SmartPulsePreferences {
  industries: string[];
  hashtags: string[];
  contentTypes: string[];
...
export const useAdvancedSmartPulse = (PulseType: 'all' | 'smart' | 'trending' = 'smart') => {
  const { user } = useAuth();
  const queryClient = useQueryClient();

  // Get user's smart Pulse preferences
  const { data: preferences, isLoading: preferencesLoading } = useQuery({
    queryKey: ['smart-Pulse-preferences', user?.id],
    queryFn: async () => {
      ...
      const prefs: SmartPulsePreferences = {
        industries: [],
        hashtags: [],
        contentTypes: [],
        excludedAuthors: [],
  ...
  // Advanced smart Pulse query with ML-inspired ranking
  const {
    data: smartPulse = [],
    ...
    queryKey: ['advanced-smart-Pulse', user?.id, PulseType, preferences, interactions],
    queryFn: async () => {
      if (!user?.id) return [];
      ...
      // Get engagement CareerAnalytics for posts
      const postIds = posts?.map(p => p.id) || [];
      const { data: CareerAnalytics } = await supabase
        .from('post_CareerAnalytics')
        .select('post_id, engagement_type, engagement_value')
        .in('post_id', postIds);

      // Build engagement map
      const engagementMap = new Map<string, number>();
      CareerAnalytics?.forEach(analytic => {
      ...
      const rankedPosts = posts?.map(post => {
        ...
        return {
          ...post,
          smart_score: score,
          engagement_CareerAnalytics: engagementScore,
  ...
  const trackEngagementMutation = useMutation({
    mutationFn: async ({
      ...
      // Record engagement CareerAnalytics
      ...
      const post = smartPulse.find(p => p.id === postId);
    ...
    onSuccess: () => {
      // Refresh Pulse to update scores
      queryClient.invalidateQueries({ queryKey: ['advanced-smart-Pulse'] });
    }
  });

  // Update Pulse preferences
  const updatePreferencesMutation = useMutation({
    mutationFn: async (newPreferences: Partial<SmartPulsePreferences>) => {
      if (!user?.id) return;
      ...
      if (prefsToInsert.length > 0) {
        await supabase.from('user_Pulse_preferences').insert(prefsToInsert);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['smart-Pulse-preferences'] });
      queryClient.invalidateQueries({ queryKey: ['advanced-smart-Pulse'] });
    }
  });
  ...
  return {
    ...
    // Loading states
    isLoading: PulseLoading || preferencesLoading,
    error: PulseError,
