...
interface SocialPulseProps {
  PulseType?: 'global' | 'following' | 'trending';
  userId?: string;
  limit?: number;
}
...
export const SocialPulse: React.FC<SocialPulseProps> = ({ 
  ...
  // Fetch posts based on Pulse type
  const { data: posts, isLoading, error, refetch } = useQuery({
    queryKey: ['social-Pulse', PulseType, userId, refreshKey],
    queryFn: async () => {
      let query = supabase
        .from('posts')
        .select(`
      ...
      // Apply filters based on Pulse type
      if (PulseType === 'following' && user) {
        // Get posts from TalentNetwork only
        ...
        // Global Pulse - show public posts
