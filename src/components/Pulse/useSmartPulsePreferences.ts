...
interface SmartPulsePreferences {
  ...
  prioritize_TalentNetwork: boolean;
...
const DEFAULT_PREFERENCES: SmartPulsePreferences = {
...
export const useSmartPulsePreferences = () => {
  const [preferences, setPreferences] = useState<SmartPulsePreferences>(DEFAULT_PREFERENCES);
  ...
  const savePreferences = async (newPreferences: Partial<SmartPulsePreferences>) => {
    setSaving(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
      ...
      toast({
        title: "Success",
        description: "Smart Pulse preferences saved successfully!",
      });
      return true;
    } catch (error) {
  ...
  const updatePreferences = (newPreferences: Partial<SmartPulsePreferences>) => {
