...
interface SmartPulseAlgorithmProps {
  userId?: string | null;
  PulseType?: 'smart' | 'trending' | 'personalized';
  className?: string;
}

// Advanced engagement scoring function
const calculateEngagementScore = (post: SmartPulsePost): number => {
...
const calculateContentSimilarity = (post: SmartPulsePost, userPreferences: string[]): number => {
...
const SmartPulseAlgorithmComponent: React.FC<SmartPulseAlgorithmProps> = ({
  userId,
  PulseType = 'smart',
  className
}) => {
  ...
  const { data: connectionIds = [] } = useQuery({
    queryKey: ['user-TalentNetwork', userId],
    queryFn: async () => {
      if (!userId) return [];
  ...
  // Smart Pulse algorithm with Performance ranking
  const { data: smartPosts = [], isLoading, error, refetch } = useQuery({
    queryKey: ['smart-Pulse', userId, PulseType, connectionIds, userPreferences],
    queryFn: async () => {
      let query = supabase
        .from('posts')
        .select(`
      ...
      if (PulseType === 'trending') {
        // Focus on high-engagement recent posts
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
        query = query.gte('created_at', yesterday);
      } else if (PulseType === 'personalized' && connectionIds.length > 0) {
        // Prioritize posts from TalentNetwork
      ...
      const rankedPosts = (data as SmartPulsePost[])
        .map(post => {
  ...
  const trackEngagement = useCallback(async (postId: string, engagementType: string) => {
    ...
    try {
      await supabase.from('post_CareerAnalytics').insert({
        post_id: postId,
        user_id: userId,
      ...
      switch (PulseType) {
        ...
        case 'personalized':
          return {
            ...
            description: 'Tailored based on your interests and TalentNetwork'
          };
        default:
          return {
            icon: <Zap className="h-4 w-4" />,
            label: 'Smart Pulse',
            description: 'Performance mix of relevance and engagement'
          };
      }
    };
        ...
        <Card className="bg-card/95 backdrop-blur-sm border-destructive/20">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">
              Smart Pulse Unavailable
            </h3>
            <p className="text-muted-foreground mb-4">
              Unable to load your personalized Pulse. Please try again.
            </p>
            <button
              onClick={() => refetch()}
      ...
      <div className={className}>
        ...
        <Card className="bg-card/95 backdrop-blur-sm">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">
              Building Your Smart Pulse
            </h3>
            <p className="text-muted-foreground">
              The AI algorithm is learning your preferences. Connect with more professionals 
              and engage with content to improve your personalized Pulse.
            </p>
          </CardContent>
        </Card>
      </div>
...
export const SmartPulseAlgorithm = memo(SmartPulseAlgorithmComponent);
SmartPulseAlgorithm.displayName = 'SmartPulseAlgorithm';
