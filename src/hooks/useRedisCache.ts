...
import { redisCache, CacheOptions } from '@/utils/redis';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (cached) {
        setData(cached);
        setLoading(false);
        await redisCache.increment('cache:hits');
        return cached;
      }

      // Cache miss - fetch fresh data
      await redisCache.increment('cache:misses');
      const freshData = await fetcher();
      
      // Cache the result
      await redisCache.set(key, freshData, options);
      setData(freshData);
