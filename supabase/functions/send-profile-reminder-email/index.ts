...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const fromEmail = Deno.env.get('SES_FROM_EMAIL') || 'TalentXcel <noreply@talentxcel.in>';
...
<body>
  <div class="container">
    ...
    <div class="body">
      ...
      <div class="cta"><a href="https://talentxcel.in">✨ Complete My Profile</a></div>
    ...
    // Send via the SMTP edge function (uses SES SMTP creds from secrets)
    ...
    const smtpResponse = await fetch(`https://${projectRef}.supabase.co/functions/v1/send-email-smtp`, {
      method: 'POST',
      headers: {
    ...
    const smtpJson = await smtpResponse.json().catch(() => ({}));
    if (!smtpResponse.ok || smtpJson?.success === false) {
      console.error('SMTP send failed:', smtpJson);
      throw new Error(smtpJson?.error || `SMTP send failed with status ${smtpResponse.status}`);
    }

    console.log('Profile reminder email sent:', { to: userEmail, messageId: smtpJson?.messageId });
