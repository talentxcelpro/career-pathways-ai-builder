...
serve(async (req) => {
  ...
  try {
    ...
    console.log('ATS Analyzer request received:', { resumeContent: !!resumeContent, targetRole, industry });
...
Provide actionable, specific feedback that will help improve ATS score and job matching.`;
    ...
    try {
      ...
      // Don't fail the response for DB errors
    }
    ...
    console.error('Error in ai-resume-analyzer function:', error);
    return new Response(JSON.stringify({ 
