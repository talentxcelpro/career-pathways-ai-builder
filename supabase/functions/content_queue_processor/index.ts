...
console.log('🚀 content_queue_processor v1.0.1: OpenAI-only with stub fallback');
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[content_queue_processor] ${req.method} ${pathname}`);
  } catch (_) {
  ...
  if (req.method === 'GET') {
    const { pathname } = new URL(req.url);
    return new Response(JSON.stringify({ ok: true, function: 'content_queue_processor', version: '1.0.1', mode: 'openai-only', path: pathname }), {
  ...
  try {
    ...
    console.error('❌ Error in content_queue_processor:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
