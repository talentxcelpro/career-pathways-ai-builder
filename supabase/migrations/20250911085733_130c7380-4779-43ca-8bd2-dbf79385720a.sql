-- Add TalentXcel domain to whitelist for internal jobs
INSERT INTO public.job_source_whitelist (domain, is_trusted, created_at) 
VALUES ('talentxcel.in', true, now()) 
...
-- Insert some Indian cities if the table exists
INSERT INTO public.job_locations_india (city, state, is_active, created_at)
VALUES 
  ('Mumbai', 'Maharashtra', true, now()),
  ('Bangalore', 'Karnataka', true, now()),
  ('Hyderabad', 'Telangana', true, now()),
  ('Delhi', 'Delhi', true, now()),
  ('Pune', 'Maharashtra', true, now()),
  ('Chennai', 'Tamil Nadu', true, now())
ON CONFLICT (city) DO NOTHING;

-- Insert 15 TalentXcel jobs with INR salaries (without external URLs to avoid validation)
