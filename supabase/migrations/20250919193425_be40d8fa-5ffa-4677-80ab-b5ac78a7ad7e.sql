-- Update existing referrals table and create comprehensive referral system

-- First, let's work with the existing referrals table structure
...
CREATE TABLE IF NOT EXISTS public.referral_events (
  ...
  referred_id UUID NULL, -- Using existing column name
...
-- Function to process referral completion (using existing column names)
CREATE OR REPLACE FUNCTION public.process_referral_completion(p_referral_code TEXT, p_referred_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  result JSONB;
BEGIN
  -- Find and update the referral (using referred_id instead of referee_id)
