-- Create resume versions table for job-specific tailoring
...
-- Create resume sections config table for user preferences
CREATE TABLE IF NOT EXISTS resume_sections_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  resume_id UUID REFERENCES ai_resumes(id) ON DELETE CASCADE,
...
-- Create section templates table for quick-start content
...
-- RLS Policies for resume_sections_config
...
-- RLS Policies for section_analytics
