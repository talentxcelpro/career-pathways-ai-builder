-- Set up automatic email triggers for welcome emails and other events
...
-- Update email event definitions to ensure they are enabled
INSERT INTO public.email_event_definitions (event_key, event_name, description, is_enabled, created_at)
VALUES 
  ('welcome', 'Welcome Email', 'Sent to new users when they register', true, NOW()),
  ('job_application_received', 'Job Application Confirmation', 'Sent when user applies for a job', true, NOW()),
  ('profile_completion_reminder', 'Profile Completion Reminder', 'Sent to users with incomplete profiles', true, NOW())
