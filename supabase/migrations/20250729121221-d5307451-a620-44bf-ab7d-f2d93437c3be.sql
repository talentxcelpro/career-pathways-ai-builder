-- Create job_applications table for user applications
...
-- Create scraping_jobs_queue table for managing scraping tasks
...
-- Add missing columns to jobs table for bot publishing
ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS posted_by_bot UUID REFERENCES ai_bots(id);
...
ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS original_post_date TIMESTAMP WITH TIME ZONE;
...
-- Add RLS policies for scraping_jobs_queue
