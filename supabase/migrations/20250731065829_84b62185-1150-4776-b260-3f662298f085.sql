...
-- Enable RLS for job views
ALTER TABLE public.job_views ENABLE ROW LEVEL SECURITY;

-- Create policy for job views
...
-- Create scraped jobs table for AI workflow
...
-- Create triggers
DROP TRIGGER IF EXISTS trigger_job_applications_stats ON public.job_applications;
CREATE TRIGGER trigger_job_applications_stats
  AFTER INSERT OR DELETE ON public.job_applications
  FOR EACH ROW EXECUTE FUNCTION update_job_stats();

DROP TRIGGER IF EXISTS trigger_job_views_stats ON public.job_views;
CREATE TRIGGER trigger_job_views_stats
...
-- Insert 20 real job samples for testing
