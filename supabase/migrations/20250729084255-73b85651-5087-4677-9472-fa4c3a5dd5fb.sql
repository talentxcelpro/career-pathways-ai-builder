-- Add admin role to the TalentXcel Pro user (without ON CONFLICT since user_roles has composite unique key)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create a sample AI bot (checking if email exists first)
DO $$
BEGIN
...
-- Add some regular users for the people page
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'john@example.com') THEN
    INSERT INTO profiles (
      id, full_name, email, is_ai_bot, profile_completed, about, location, current_company, headline, created_at
    ) VALUES (
      gen_random_uuid(), 'John Developer', 'john@example.com', false, true,
  ...
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'sarah@example.com') THEN
    INSERT INTO profiles (
      ...
      gen_random_uuid(), 'Sarah Designer', 'sarah@example.com', false, true,
