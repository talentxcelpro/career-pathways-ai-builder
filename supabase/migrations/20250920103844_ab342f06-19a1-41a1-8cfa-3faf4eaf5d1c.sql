-- Fix the user scores ranking system and add comprehensive TXC automation
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM txc_transactions 
      WHERE user_id = application_record.user_id 
      AND transaction_type = 'mining' 
      AND description = 'Apply to a job'
      AND created_at > application_record.applied_at - INTERVAL '1 hour'
    ) THEN
  ...
  LOOP
    IF NOT EXISTS (
      ...
      AND created_at > connection_record.created_at - INTERVAL '30 minutes'
    ) THEN
