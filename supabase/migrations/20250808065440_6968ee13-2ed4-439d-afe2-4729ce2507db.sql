-- Function to clean up test users and duplicates (FINAL CORRECTED VERSION)
...
BEGIN
  ...
  IF test_profile_ids IS NOT NULL AND array_length(test_profile_ids, 1) > 0 THEN
    ...
    WHERE created_by = ANY(test_profile_ids);
    GET DIAGNOSTICS batch_count = ROW_COUNT;

    -- Delete user roles for test users (if user_roles references profiles.id)
    ...
    -- Delete career passport entries (if table exists)
