-- Phase 1: Database Performance Optimization (Corrected column names)
...
-- Job applications optimization (using correct column name)
...
-- Saved jobs optimization (using correct column name)
CREATE INDEX IF NOT EXISTS idx_saved_jobs_user_date 
ON public.saved_jobs (user_id, saved_at DESC);

-- Posts optimization for Network page (using correct column name)
...
ON public.posts (visibility, posted_at DESC) 
