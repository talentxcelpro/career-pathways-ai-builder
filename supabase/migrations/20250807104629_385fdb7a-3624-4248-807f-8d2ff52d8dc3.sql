-- Fix the get_jobs_paginated_optimized function to avoid column ambiguity
CREATE OR REPLACE FUNCTION public.get_jobs_paginated_optimized(p_page integer DEFAULT 1, p_limit integer DEFAULT 20, p_search text DEFAULT ''::text, p_location text DEFAULT ''::text, p_employment_types text[] DEFAULT '{}'::text[], p_experience_levels text[] DEFAULT '{}'::text[], p_min_salary integer DEFAULT 0, p_max_salary integer DEFAULT 0, p_is_remote boolean DEFAULT false, p_skills text[] DEFAULT '{}'::text[], p_sort_by text DEFAULT 'created_at'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $$
DECLARE
  ...
  result_json JSONB;
BEGIN
  -- Return jobs with filters and pagination using a variable
  WITH filtered_jobs AS (
    SELECT 
      ...
      c.is_verified as company_verified_flag
    FROM public.jobs j
    ...
    WHERE 
      ...
      -- Search filter (use j.company_name to avoid ambiguity)
      AND (p_search = '' OR (
        ...
        jsonb_agg(
          jsonb_build_object(
            ...
            'companies', jsonb_build_object(
              ...
              'is_verified', COALESCE(pj.company_verified_flag, false)
            )
          )
