-- PHASE 1: Database optimization for ultra-fast jobs scaling
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_active_status_expires 
...
WHERE is_active = true AND (salary_min > 0 OR salary_max > 0);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_location_text 
ON public.jobs USING gin (to_tsvector('english', location)) 
WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_title_company_search 
ON public.jobs USING gin (to_tsvector('english', title || ' ' || company_name)) 
WHERE is_active = true;
...
-- 5. Create materialized view for job statistics (updated hourly)
CREATE MATERIALIZED VIEW IF NOT EXISTS job_stats_materialized AS
SELECT 
  COUNT(*) as total_jobs,
...
FROM public.jobs 
WHERE is_active = true AND job_status = 'open' AND expires_at > NOW();

-- Create unique index for materialized view refresh
CREATE UNIQUE INDEX IF NOT EXISTS job_stats_materialized_unique_idx 
ON job_stats_materialized ((1));

-- 6. Function to refresh job stats
CREATE OR REPLACE FUNCTION refresh_job_stats()
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY job_stats_materialized;
END;
$$;