-- Phase 3: Advanced CRM & Analytics Enhancement (Final Fix)
...
-- Create client communication log (corrected to use profile_id)
CREATE TABLE IF NOT EXISTS pro_communications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  profile_id UUID REFERENCES pro_service_profiles(id) ON DELETE CASCADE,
  lead_id UUID REFERENCES pro_leads(id) ON DELETE CASCADE,
...
-- Create pro performance metrics (corrected to use profile_id)
...
-- Create client feedback and reviews (corrected to use profile_id)
...
-- Create service provider notes (corrected to use profile_id)
...
-- RLS Policies for pro_lead_scoring (corrected to use profile_id)
...
-- RLS Policies for pro_communications (corrected column name)
...
-- RLS Policies for pro_performance_metrics (corrected column name)
...
-- RLS Policies for pro_client_feedback (corrected column name)
...
-- RLS Policies for pro_client_notes (corrected column name)
...
CREATE INDEX IF NOT EXISTS idx_pro_communications_profile_id ON pro_communications(profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_communications_lead_id ON pro_communications(lead_id);
CREATE INDEX IF NOT EXISTS idx_pro_performance_metrics_profile_id ON pro_performance_metrics(profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_performance_metrics_date ON pro_performance_metrics(metric_date);
CREATE INDEX IF NOT EXISTS idx_pro_client_feedback_profile_id ON pro_client_feedback(profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_client_feedback_client_id ON pro_client_feedback(client_id);
CREATE INDEX IF NOT EXISTS idx_pro_client_notes_profile_id ON pro_client_notes(profile_id);
...
INSERT INTO pro_performance_metrics (profile_id, metric_type, metric_value, metric_date) 
SELECT 
  id, 
  'revenue',
  RANDOM() * 5000 + 1000,
