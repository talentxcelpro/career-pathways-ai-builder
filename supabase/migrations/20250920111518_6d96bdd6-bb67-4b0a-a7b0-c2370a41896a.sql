-- Fix missing token_transactions table and implement unified gamification system

-- Create token_transactions table (missing table causing errors)
...
-- Function to update user TXC balance
CREATE OR REPLACE FUNCTION update_user_txc_balance(
  p_user_id UUID,
  p_amount INTEGER,
  ...
  LOOP
    ...
    BEGIN
      ...
      IF is_earned AND NOT COALESCE(achievement_record.earned, false) THEN
        -- Award TXC points
        PERFORM update_user_txc_balance(
          p_user_id, 
