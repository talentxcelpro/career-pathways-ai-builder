...
CREATE POLICY "Employers can view candidates they have access to" 
ON public.candidates 
FOR SELECT 
USING (
  auth.uid() IN (
    SELECT posted_by FROM jobs 
    WHERE jobs.id IN (
      SELECT job_id FROM job_applications 
      WHERE job_applications.user_id = (
        SELECT user_id FROM profiles WHERE id = candidates.profile_id
      )
    )
...
-- 7. Fix function search paths (security warning)
-- Update functions without proper search_path
CREATE OR REPLACE FUNCTION public.validate_admin_operation(_required_role text DEFAULT 'admin')
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  RETURN EXISTS (
    ...
    AND role::text = _required_role 
    AND is_active = true
  );
END;
$$;

-- 8. Create missing RLS policies for remaining tables
