-- Complete Company Dashboard Schema with Real-time Features
...
CREATE TABLE IF NOT EXISTS company_integrations (
  ...
  api_credentials JSONB DEFAULT '{}', -- This should be encrypted in production
...
CREATE OR REPLACE FUNCTION update_company_realtime_metrics()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  -- Update engagement metrics when posts or interactions change
  IF TG_TABLE_NAME IN ('company_posts', 'company_post_interactions', 'company_follows') THEN
    INSERT INTO company_realtime_metrics (company_id, metric_type, metric_value, metadata)
    VALUES (
      COALESCE(NEW.company_id, OLD.company_id),
      'engagement_update',
      calculate_company_engagement_score(COALESCE(NEW.company_id, OLD.company_id)),
      jsonb_build_object('trigger_table', TG_TABLE_NAME, 'operation', TG_OP)
    );
  END IF;
...
CREATE TRIGGER company_posts_metrics_trigger
  AFTER INSERT OR UPDATE OR DELETE ON company_posts
  FOR EACH ROW EXECUTE FUNCTION update_company_realtime_metrics();

CREATE TRIGGER company_interactions_metrics_trigger
  AFTER INSERT OR UPDATE OR DELETE ON company_post_interactions
  FOR EACH ROW EXECUTE FUNCTION update_company_realtime_metrics();

CREATE TRIGGER company_follows_metrics_trigger
  AFTER INSERT OR UPDATE OR DELETE ON company_follows
  FOR EACH ROW EXECUTE FUNCTION update_company_realtime_metrics();

-- Indexes for performance
CREATE INDEX idx_company_realtime_metrics_company_time ON company_realtime_metrics(company_id, timestamp DESC);
CREATE INDEX idx_company_ai_recommendations_company_type ON company_ai_recommendations(company_id, recommendation_type);
CREATE INDEX idx_company_content_calendar_company_date ON company_content_calendar(company_id, scheduled_date);
CREATE INDEX idx_company_analytics_sessions_company_date ON company_analytics_sessions(company_id, session_date DESC);
CREATE INDEX idx_team_collaboration_logs_company_time ON team_collaboration_logs(company_id, timestamp DESC);
