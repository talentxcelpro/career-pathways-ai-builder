-- Fix security issues with correct column names

-- Create missing RLS policies for news articles with correct column reference
...
FOR SELECT USING (published_status = 'published' OR published_status IS NULL);
...
-- Groups table policies (check if visibility column exists)
...
FOR SELECT USING (created_by = auth.uid() OR true); -- Allow all for now
