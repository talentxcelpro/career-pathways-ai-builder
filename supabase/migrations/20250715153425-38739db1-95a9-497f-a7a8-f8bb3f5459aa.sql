-- Phase 3: Advanced CRM & Analytics Enhancement (Fixed)
...
-- Create client communication log (fixed reference)
CREATE TABLE IF NOT EXISTS pro_communications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  provider_profile_id UUID REFERENCES pro_service_profiles(id) ON DELETE CASCADE,
  lead_id UUID REFERENCES pro_leads(id) ON DELETE CASCADE,
...
-- Create pro performance metrics (fixed reference)
...
-- Create client feedback and reviews (fixed reference)
...
-- Create service provider notes (fixed reference)
...
-- RLS Policies for pro_communications (fixed column name)
...
-- RLS Policies for pro_performance_metrics (fixed column name)
...
-- RLS Policies for pro_client_feedback (fixed column name)
...
-- RLS Policies for pro_client_notes (fixed column name)
...
CREATE INDEX IF NOT EXISTS idx_pro_communications_provider_id ON pro_communications(provider_profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_communications_lead_id ON pro_communications(lead_id);
CREATE INDEX IF NOT EXISTS idx_pro_performance_metrics_provider_id ON pro_performance_metrics(provider_profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_performance_metrics_date ON pro_performance_metrics(metric_date);
CREATE INDEX IF NOT EXISTS idx_pro_client_feedback_provider_id ON pro_client_feedback(provider_profile_id);
CREATE INDEX IF NOT EXISTS idx_pro_client_feedback_client_id ON pro_client_feedback(client_id);
CREATE INDEX IF NOT EXISTS idx_pro_client_notes_provider_id ON pro_client_notes(provider_profile_id);
...
INSERT INTO pro_performance_metrics (provider_profile_id, metric_type, metric_value, metric_date) 
SELECT 
  id, 
  'revenue',
  RANDOM() * 5000 + 1000,
