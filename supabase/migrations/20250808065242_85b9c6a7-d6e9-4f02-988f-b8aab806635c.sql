-- Function to clean up test users and duplicates (CORRECTED)
...
DECLARE
  ...
  test_user_ids UUID[];
  test_profile_user_ids UUID[];
BEGIN
  -- Get all test profile IDs and their corresponding user_ids
  SELECT ARRAY_AGG(id), ARRAY_AGG(user_id) INTO test_user_ids, test_profile_user_ids
  FROM profiles 
  WHERE 
    ...
    email ILIKE '%test%@upload.local' OR
  ...
  -- Log what we found
  RAISE NOTICE 'Found % test profiles to delete', COALESCE(array_length(test_user_ids, 1), 0);

  -- Delete related CV files first (using user_id, not profile_id)
  DELETE FROM cv_files 
  WHERE user_id = ANY(test_profile_user_ids);
  GET DIAGNOSTICS cv_count = ROW_COUNT;

  -- Delete bulk upload batches created by test users
  DELETE FROM bulk_upload_batches 
  WHERE created_by = ANY(test_profile_user_ids);
