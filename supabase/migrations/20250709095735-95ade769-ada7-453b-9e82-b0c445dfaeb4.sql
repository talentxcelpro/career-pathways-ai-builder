-- Phase 1: Enhanced Resume Builder Database Structure
...
-- Create resume_analytics for tracking
...
-- Create ats_analysis table for detailed ATS scoring
CREATE TABLE IF NOT EXISTS public.resume_ats_analysis (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  resume_id UUID REFERENCES public.ai_resumes(id) ON DELETE CASCADE NOT NULL,
...
-- RLS policies for resume_ats_analysis
CREATE POLICY "Users can view their ATS analysis" ON public.resume_ats_analysis
FOR ALL USING (
  resume_id IN (
    SELECT id FROM public.ai_resumes WHERE user_id = auth.uid()
  )
...
('Modern Professional', 'professional', 'Contemporary design with professional appeal', '{"columns": 2, "header_style": "modern", "spacing": "compact"}', false, 90, ARRAY['modern', 'professional', 'creative'], 'ModernTemplate', '{"fontFamily": "Inter", "primaryColor": "#059669"}', '/templates/modern-professional.jpg'),
...
('DevOps Engineer', 'technical', 'Cloud and DevOps focused template', '{"columns": 1, "header_style": "minimal", "spacing": "compact"}', false, 92, ARRAY['devops', 'cloud', 'infrastructure'], 'DevOpsTemplate', '{"fontFamily": "Ubuntu", "primaryColor": "#f59e0b"}', '/templates/devops-engineer.jpg'),
...
('PhD Graduate', 'academic', 'Perfect for PhD holders entering industry', '{"columns": 1, "header_style": "formal", "spacing": "standard"}', false, 89, ARRAY['phd', 'graduate', 'academic'], 'PhDTemplate', '{"fontFamily": "Libre Baskerville", "primaryColor": "#059669"}', '/templates/phd-graduate.jpg'),
