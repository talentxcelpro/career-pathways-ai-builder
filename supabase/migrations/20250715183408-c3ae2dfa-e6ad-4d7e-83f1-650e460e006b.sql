...
-- Add constraint to ensure vanity URLs are lowercase and alphanumeric with dashes
ALTER TABLE public.profiles ADD CONSTRAINT IF NOT EXISTS chk_vanity_url_format 
CHECK (vanity_url ~ '^[a-z0-9-]+$' AND length(vanity_url) >= 3 AND length(vanity_url) <= 50);
