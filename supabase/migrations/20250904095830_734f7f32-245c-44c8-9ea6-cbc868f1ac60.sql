-- Batch 14C: Fix Function Search Path Mutable warnings without dropping functions

-- Update functions by recreating them with SET search_path (without changing signatures)

-- Update create_notification function to include SET search_path
...
-- Add SET search_path to common trigger functions
...
EXCEPTION
  WHEN undefined_table THEN
    -- Ignore if career_passport table doesn't exist
    RETURN NEW;
END;
$$;