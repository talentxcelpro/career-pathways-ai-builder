...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_location_active_posted 
ON public.jobs (location, is_active, posted_at DESC) 
WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_search_active 
ON public.jobs USING gin(to_tsvector('english', title || ' ' || COALESCE(description, '') || ' ' || COALESCE(company_name, ''))) 
WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_salary_active 
ON public.jobs (salary_max DESC, is_active) 
WHERE is_active = true AND salary_max IS NOT NULL;

-- Profiles table optimization
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_username_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companies_slug_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_applications_job_status 
