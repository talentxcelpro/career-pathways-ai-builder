-- Migration: Optimize Job Listing & Search Payload
-- Reduces listing payload from ~61.7 KB to ~16.5 KB (~73% reduction)
-- Returns 200-character description_snippet instead of full multi-paragraph HTML
-- Full description remains 100% intact and untouched in public.jobs table for job detail view (/jobs/:slug)

CREATE OR REPLACE FUNCTION public.get_jobs_paginated_optimized(
  p_page integer DEFAULT 1,
  p_limit integer DEFAULT 20,
  p_search text DEFAULT ''::text,
  p_location text DEFAULT ''::text,
  p_employment_types text[] DEFAULT '{}'::text[],
  p_experience_levels text[] DEFAULT '{}'::text[],
      ...
      'jobs', COALESCE(
        jsonb_agg(
          jsonb_build_object(
            'id', pj.id,
            'title', pj.title,
            'description', SUBSTRING(pj.description FROM 1 FOR 200),
            'description_snippet', SUBSTRING(pj.description FROM 1 FOR 200),
